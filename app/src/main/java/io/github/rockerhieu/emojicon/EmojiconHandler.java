...
//        Emoji.fromChars("\u0023\u20e3"),
  ...
  public static void addEmojis(Context context, Spannable text, int emojiSize, int emojiAlignment,
    ...
    int textLengthToProcess =
        length < 0 || length >= textLengthToProcessMax ? textLength : (length + index);
    ...
    for (int i = index; i < textLengthToProcess; i += skip) {
      ...
      if (icon == 0) {
        ...
        if (i + skip < textLengthToProcess) {
          ...
          if (followUnicode == 0xfe0f) {
            ...
            //some older versions of iOS don't use a combining character, instead it
            // just goes straight to the second part
            int followSkip = Character.charCount(followUnicode);
            ...
            //TODO seems like we could do this for every emoji type rather than
            // having that giant static map, maybe this is too slow?
            ...
            if (sEmojisModifiedMap.containsKey(resourceName)) {
              ...
              resourceId = context.getResources().getIdentifier(resourceName, "drawable", context
      ...
      if (icon > 0 && i >= 0) {
        text.setSpan(new EmojiconSpan(context, icon, emojiSize, emojiAlignment, textSize), i,
            i + skip, Spannable
  ...
  public static List<TextEmoji> getEmojis(Context context, String text) {
    ...
    for (int i = index; i < textLengthToProcess; i += skip) {
      ...
      if (isSoftBankEmoji(c)) {
        ...
        icon = getSoftbankEmojiResource(c);
        skip = icon == 0 ? 0 : 1;
        if (i == textLengthToProcess - 1) {
          iconText = text.substring(i);
        } else {
          iconText = text.substring(i, i + skip);
        }
      }
      ...
      if (icon == 0) {
        int unicode = Character.codePointAt(text, i);
        skip = Character.charCount(unicode);
        if (text.length() == 1 || i == textLengthToProcess - 1) {
          if (text.length() == 1 || skip == 1) {
            iconText = text.substring(i);
          } else {
            iconText = text.substring(i, i + 1);
          }
        } else {
          iconText = text.substring(i, i + skip);
        }
      ...
      if (icon > 0) {
        //textEmoji.emoji = new TextView(context);
        //textEmoji.emoji.setText(new String(Character.toChars(Character.codePointAt(text, i))), TextView.BufferType
        // .SPANNABLE);
        //textEmoji.emoji = Spannable.Factory.getInstance().newSpannable(new String(Character.toChars(Character.codePointAt
        // (text, i))));
//                textEmoji.emojiTextView = new TextView(context);
//                textEmoji.emojiTextView.setText(new String(Character.toChars(Character.codePointAt(text, i))), TextView.BufferType
// .SPANNABLE);

        textEmoji.icon = context.getResources().getDrawable(icon);
//                String s = StringUtils.unicode2String(c + "");
        textEmoji.text = iconText;
      } else {
        textEmoji.text = text.subSequence(i, i + skip).toString();
      }
...
//        public Bitmap iconBitmap;
    ...
    public void setSize(int height) {
      if (icon != null) {
        try {
          width = height * icon.getIntrinsicWidth() / icon.getIntrinsicHeight();
          icon.setBounds(0, 0, width, height);
//                    emojiTextView.measure(width, height);
//                    emojiTextView.layout(0, 0, width, height);
...
//                    emojiTextView.draw(canvas);
    ...
    public void onDraw(Canvas canvas, Paint paint, Paint strokepaint, int dx, String startColorStr,
        String endColorStr, int
        lineSpacing) {
      if (!ready) {
        return;
      }
      if (icon != null) {//TextUtils.isEmpty(text) ||
        ...
        icon.draw(canvas);
//                canvas.scale(4.8f, 4.8f);
//               canvas.drawBitmap(iconBitmap, 0, 0 ,null);
        canvas.restore();
      } else {
//                Log.d("startColorStr", startColorStr + "," + endColorStr);
        if (!TextUtils.isEmpty(startColorStr) || !TextUtils.isEmpty(endColorStr)) {
          setLinearGradient(paint, startColorStr, endColorStr, fontTotalWidth / 2, offsetY,
              fontTotalWidth / 2, offsetY +
                  fontTotalHeight + lineSpacing / 2);
        } else {
        ...
        if (text.equals("\n")) {
          ...
          if (strokepaint.getStrokeWidth() > paint.getStrokeWidth() + 0.01) {
            if (paint.getAlpha() < 255) {
              ...
              if (fontTotalHeight == 0 || fontTotalWidth == 0) {
                return;
              }
              Bitmap bmSrc = Bitmap
                  .createBitmap(fontTotalWidth * 3 / 2, fontTotalHeight * 3 / 2, Bitmap.Config
                      .ARGB_8888);
              ...
              canvas.drawText(text, fontTotalWidth / 2 + offsetX, fontTotalHeight - dx + offsetY,
                  strokepaint);//画出描边
            }
          }
...
//                    Log.d("drawTextRow字", text);
  ...
  public static void setLinearGradient(Paint paint, String startcolorStr, String endcolorStr,
    ...
    float[] positions = {};
    //更适合多行文字的效果
    LinearGradient linearGradient = new LinearGradient(x0, y0, x1, y1, startcolor, endcolor,
        Shader.TileMode.CLAMP);
    //这里的y1不够准，写y,g等字体会不够用，用CLAMP格式最好。
